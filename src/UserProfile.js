...
import generic_avatar from './generic-avatar.png'
...
function UserProfile() {
  ...
  console.log(userReviews)
    ...
    useEffect(() =>{
        ...
        let res = await axios.get(`/users/${userHandle}`);
        if(userHandle !== currUser.isUser){
          let votes = await axios.get(`/users/${currUser.isUser}/votes`);
      ...
      async function addStar(){
        ...
        let result = await axios.post(`/users/${userDetails.username}/add-star`, 
      ...
      async function removeStar(){
        let result = await axios.post(`/users/${userDetails.username}/remove-star`,{
            userVoting: currUser.isUser
        }
        )
        let row1= result.data[0];
      ...
      async function handleBioSubmit(evt){
          ...
          let res = await axios.patch(`/users/${userHandle}/bio`, {newBio});
          let row1 = res.data[0]
          let {bio, avatar, stars, username} = row1;
      ...
      async function handleAvatarSubmit(evt){
          ...
          let res = await axios.patch(`/users/${userHandle}/avatar`, {newAvatar});
          let row1 = res.data[0];
          let {bio, avatar, stars, username} = row1;
        setUserDetails({bio, avatar, stars, username})
